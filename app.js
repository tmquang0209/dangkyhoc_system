...
const apiEndpoints = {
    subjectList: "/controller/schedule.php?subjectList",
    fee: "/controller/schedule.php?getFee",
        ...
        const del = async () => {
            try {
                const response = await fetch(apiEndpoints.removeFromDatabase, {
                    ...
                    body: JSON.stringify({ studentCode, data }), // Convert data to JSON
                });
    ...
    if (storageData.length == 0) {
        ...
        storageData.forEach((element) => {
            ...
            html += ` <tr>
                        ...
                        <td style="max-width:5px">${element.Credits}</td>
                        <td style="max-width:50px"><input style="width:100%" type="number" disabled name="creditFactor" id="creditFactor" data-credits="${element.Credits}" value="${element.Coef}" onkeyup="sumCreditFactor()"></td>
                        <td>${teachers}</td>
                    </tr>`;
    ...
    try {
        const getFee = async () => {
            ...
            const response = await fetch(apiEndpoints.fee, {
                ...
                body: JSON.stringify({ semesterid }), // Convert the data to a JSON string
            });
